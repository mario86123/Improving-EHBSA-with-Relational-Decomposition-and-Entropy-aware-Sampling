// Adaptive cut point New RBOP (can add new edge)
 ...
 * Class constructor.
 */
CACNRBOP::CACNRBOP(int problem_size, int sel_size, double b_ratio)
{
 ...
 */
// bool CACNRBOP::Learn(CPopulation * population, int size)
//
// mode == 1 -> upper half population
// mode == 2 -> lower half population
// mode == 3 -> whole population
bool CACNRBOP::Learn(CPopulation * population, int mode, int * used_distance_count_arr)
{   
    // population->Print();
    // cout << endl;
    ...
    for(int dis = 0; dis < m_problem_size; ++dis) {
        used_distance_count_arr[dis] += m_used_distance_count[dis];
 ...
 */
void CACNRBOP::Sample(int * genes, int * templateee, int cut_point_count)
{
...
edgeee CACNRBOP::add_edgesss(int sampled_node_num, int sampled_node_idx, list<edgeee>& edge_lst, bool * is_sampled_node, bool * is_sampled_idx) {
...
edgeee CACNRBOP::Build_network(int * templateee, int already_sampled_idx_count, list<edgeee>&  edge_lst, int * sampled_or_not_idx, bool * is_sampled_node) {
...
double CACNRBOP::fast_calculate_entropy(int * arr, int summation) {
...
// update edges information and delete unsable edgesss
edgeee CACNRBOP::update_and_find_lowest_entropy_edge(int sample_idx, int sample_node_num, list<edgeee>&  edge_lst) {
...
void CACNRBOP::construct_probability_array(double * tmp_probability_array, bool * is_sampled_node, edgeee lowest_entropy_edge) {
