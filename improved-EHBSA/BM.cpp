 ...
 * Class constructor.
 */
CBM::CBM(int problem_size, int sel_size, double b_ratio, int cut_point_count)
{
    ...
    m_relation_count_used_in_sampling = m_problem_size / m_cut_point_count;
    m_used_binary_relation = new int*[m_relation_count_used_in_sampling];
    for (int i = 0; i < m_relation_count_used_in_sampling; ++i) {
        m_used_binary_relation[i] = new int[3]();
    }
...
CBM::~CBM()
{
    ...
    for (int i = 0; i < m_relation_count_used_in_sampling; ++i) {
        delete[] m_used_binary_relation[i];
    }
    delete[] m_used_binary_relation;
 ...
 */
bool CBM::Learn(CPopulation * population, int size)
{   
    // population->Print();
    // cout << endl;
 ...
 */
void CBM::Sample(int * genes, int * templateee, int **used_binary_relation)
{
    ...
    for (int kkk = 0; kkk < m_problem_size - already_sampled_idx_count; kkk++) {
        ...
        // ref node
        used_binary_relation[kkk][0] = edge_lst[lowest_entropy_edge_num].node_a;
        // distance
        used_binary_relation[kkk][1] = edge_lst[lowest_entropy_edge_num].distance;
        // sample node num
        used_binary_relation[kkk][2] = genes[sample_idx];
    ...
    // used_binary_relation =  m_used_binary_relation;
...
int CBM::Build_network(int * templateee, int already_sampled_idx_count, edgeee * edge_lst, int * sampled_or_not_idx, bool * is_sampled_node) {
...
double CBM::fast_calculate_entropy(double * arr, double summation) {
...
int CBM::update_and_find_lowest_entropy_edge(int sample_idx, int sample_node_num, int total_edge_count, edgeee * edge_lst) {
...
void CBM::construct_probability_array(double * tmp_probability_array, bool * is_sampled_node, int lowest_entropy_edge_num, edgeee * edge_lst) {
