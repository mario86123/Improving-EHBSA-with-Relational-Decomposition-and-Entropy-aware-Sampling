 ...
 * Class constructor.
 */
CFOUR::CFOUR(int problem_size, int sel_size, double b_ratio)
{
 ...
 */
// bool CFOUR::Learn(CPopulation * population, int size)
//
// mode == 1 -> upper half population
// mode == 2 -> lower half population
// mode == 3 -> whole population
bool CFOUR::Learn(CPopulation * population, int start_idx, int end_idx)
{   
...
// 0: sequential, cut = 3, 1: random, , cut = 10
void CFOUR::Sample(int * genes, int * templateee, int cut_point_and_random_sequential)
{
    int cut_point_count = -1;

    if (cut_point_and_random_sequential == 0) {
        cut_point_count = 3;
    }
    else { // (cut_point_and_random_sequential == 1)
        cut_point_count = 10;
    }


    int already_sampled_idx_count = m_problem_size - (m_problem_size / cut_point_count);
    if (cut_point_count == 1) {
    ...
    if (cut_point_and_random_sequential == 1) {
        for (int i = 0; i < m_problem_size; ++i) {
    ...
    else { // cut_point_and_random_sequential == 0
...
edgeee CFOUR::add_edgesss(int sampled_node_num, int sampled_node_idx, list<edgeee>& edge_lst, bool * is_sampled_node, bool * is_sampled_idx) {
...
edgeee CFOUR::Build_network(int * templateee, int already_sampled_idx_count, list<edgeee>&  edge_lst, int * sampled_or_not_idx, bool * is_sampled_node) {
...
double CFOUR::fast_calculate_entropy(int * arr, int summation) {
...
// update edges information and delete unsable edgesss
edgeee CFOUR::update_and_find_lowest_entropy_edge(int sample_idx, int sample_node_num, list<edgeee>&  edge_lst) {
...
void CFOUR::construct_probability_array(double * tmp_probability_array, bool * is_sampled_node, edgeee lowest_entropy_edge) {
