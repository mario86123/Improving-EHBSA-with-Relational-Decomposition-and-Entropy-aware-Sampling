 ...
 * Class constructor.
 */
CMST2ME::CMST2ME(int problem_size, int sel_size, double b_ratio, string result_file_name)
{
	m_problem_size=problem_size;
    m_sample_size=sel_size;
    m_samples= new int*[m_sample_size];
    m_max_num_of_edge = 10 * m_problem_size;
    ...
    // cout << "eps: " << m_epsilon << endl;

    m_mst_edge_arr = new edge[m_max_num_of_edge];
    ...
    useless_counter = new int[m_problem_size]();
    // cout << "fff";
...
CMST2ME::~CMST2ME()
{
    ...
    for(int i = 0; i < m_problem_size; ++i) {
        // cout << "useless_counter[" << i << "]: " << useless_counter[i] << endl;
    }    
 ...
 */
bool CMST2ME::Learn(CPopulation * population, int size)
{   
    ...
    bool is_selected_edge[total_edge_num];
    int kkk = 0;
    // union find
    ...
    for (edge e: all_edge) {
        ...
        if (find_set(e.node_a, node_set) != find_set(e.node_b, node_set) && mst_edge_count < m_problem_size - 1) {

            // add that edge to graph
            union_set(e.node_a, e.node_b, node_set);
            m_mst_edge_arr[mst_edge_count] = e;
            mst_edge_count ++;
            is_selected_edge[kkk] = true;

            // if (mst_edge_count >= m_problem_size - 1) break; // change here !!
        }

        else {
            is_selected_edge[kkk] = false;
        }
        kkk ++;
    }
    ...
    for(int i = 0; i < total_edge_num; ++i) {
        ...
        if (is_selected_edge[i] == false) {
            m_mst_edge_arr[mst_edge_count] = all_edge[i];
            mst_edge_count ++;
        }
        if (mst_edge_count >= m_max_num_of_edge) {
            break;
        }
    }
    ...
    for (int i = 0; i < m_max_num_of_edge; ++i) {
        ...
        // cout << m_mst_edge_arr[i].node_a << " " << m_mst_edge_arr[i].node_b << endl;
    ...
    // cout << "m_mst_edge_arr[18].node_a: "<<m_mst_edge_arr[18].node_a<<endl;

    // calculate node entropy ratio
    for (int i = 0; i < m_problem_size; ++i) {
        // cout << "283: m_mst_edge_arr[18].node_a: "<<m_mst_edge_arr[18].node_a<<endl;

        // m_mst_node_arr[i].node_num = i;
        // cout << "286: m_mst_edge_arr[18].node_a: "<<m_mst_edge_arr[18].node_a<<endl;
...
void CMST2ME::Sample(int * genes)
{
    ...
    for (int num = 0; num < m_max_num_of_edge ; num++) {
        ...
        // cout <<"a: " <<m_mst_edge_arr[num].node_a<<endl;

        mst_edge_lst[num].node_b =  m_mst_edge_arr[num].node_b;
        // cout << "mst_edge_lst["<<num<<"].status: "<<mst_edge_lst[num].status<<", a:"<<mst_edge_lst[num].node_a<<", b:"<<mst_edge_lst[num].node_b<<endl;
        ...
        mst_edge_lst[num].distance = m_mst_edge_arr[num].distance;
    ...
    int sample_idx = sample_from_array(m_node_matrix[min_entropy_node_num], m_problem_size);

    genes[sample_idx] = min_entropy_node_num;
    // cout << "genes["<<sample_idx<<"] = " << min_entropy_node_num << endl;

    // ===== update information
    // already_sampled_index_arr[m_problem_size]; // store aleady sampled index
    is_sampled_node[min_entropy_node_num] = true;
    ...
    for (int edge_num = 0; edge_num < m_max_num_of_edge; edge_num ++) {
        ...
        if (mst_edge_lst[edge_num].node_a == min_entropy_node_num || mst_edge_lst[edge_num].node_b == min_entropy_node_num ) {
        ...
        // (status 0 -> 1) or (status 0 -> 2)
            ...
            if (mst_edge_lst[edge_num].node_a == min_entropy_node_num) {
                ...
                for (int i = 0; i < m_problem_size; i ++) {
                    ...
                    if (is_sampled_index[tmp_idx] == false) { // that index is not sampled yet
                        tmp_arr[tmp_idx] = m_edge_matrix[min_entropy_node_num][mst_edge_lst[edge_num].node_b][i];
                        tmp_sum += m_edge_matrix[min_entropy_node_num][mst_edge_lst[edge_num].node_b][i];
                    }
            ...
            else { // mst_edge_lst[edge_num].node_b == min_entropy_node_num
                ...
                for (int i = 0; i < m_problem_size; i ++) {
                    ...
                    if (is_sampled_index[tmp_idx] == false) { // that index is not sampled yet
                        tmp_arr[tmp_idx] = m_edge_matrix[min_entropy_node_num][mst_edge_lst[edge_num].node_a][i];
                        tmp_sum += m_edge_matrix[min_entropy_node_num][mst_edge_lst[edge_num].node_a][i];
                    }
                    else  {// that index is already sampled
                        tmp_arr[tmp_idx] = 0.0;
                    }
    ...
    for (int sample_count = 1; sample_count < m_problem_size; sample_count++) {
        ...
        for (int i = 0; i < m_max_num_of_edge; i ++) {
            ...
            if (mst_edge_lst[i].status == 1 || mst_edge_lst[i].status == 2 ) {
                // cout << "mst_edge_lst["<<i<<"].status: "<<mst_edge_lst[i].status<<", a:"<<mst_edge_lst[i].node_a<<", b:"<<mst_edge_lst[i].node_b<<", entropy: "<< mst_edge_lst[i].entropy<<endl;
        ...
        int sample_node_num, ref_node_num;
        ...
        if (mst_edge_lst[min_entropy_edge_num].status == 1) { // node_a is already sampled
            ...
            // cout << "ref_node_num: " << ref_node_num <<", sample_node_num: "<<sample_node_num  << endl;
        ...
        for (int i = 0; i < m_problem_size; i ++) {
            ...
            if (is_sampled_index[tmp_idx] == false) { // that index is not sampled yet
                ...
                // cout << "ref_node_num: " <<ref_node_num << endl;
                // cout << "m_edge_matrix["<<ref_node_num<<"]["<<sample_node_num<<"]["<<i<<"]: ";
                // cout << m_edge_matrix[ref_node_num][sample_node_num][i]<<endl;
                sample_arr[tmp_idx] = m_edge_matrix[ref_node_num][sample_node_num][i];
                // cout << "sample_arr["<<tmp_idx<<"]: " <<sample_arr[tmp_idx]<<endl;
            }
            else  {// that index is already sampled
                sample_arr[tmp_idx] = 0.0;

                if (i == mst_edge_lst[min_entropy_edge_num].distance) {
                    // cout << "sample count: " << sample_count << ", MST useless !!" << endl;
                    useless_counter[sample_count] ++;
                }
            }
        }
