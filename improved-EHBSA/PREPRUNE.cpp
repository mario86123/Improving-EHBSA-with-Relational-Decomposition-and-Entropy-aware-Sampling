 ...
 * Class constructor.
 */
CPREPRUNE::CPREPRUNE(int problem_size, int sel_size, double b_ratio)
{
    ...
    m_can_used_distance_arr = new bool[m_problem_size]();
...
CPREPRUNE::~CPREPRUNE()
{
    ...
    delete [] m_can_used_distance_arr;
 ...
 */
// bool CPREPRUNE::Learn(CPopulation * population, int size)
//
// mode == 1 -> upper half population
// mode == 2 -> lower half population
// mode == 3 -> whole population
bool CPREPRUNE::Learn(CPopulation * population, int mode)
{   
    // population->Print();
    // cout << endl;
    ...
    double distance_entropy_arr[m_problem_size];
    // calculate the entropy of each distance
    distance_entropy_arr[0] = 0;
    for (int distance = 1; distance < m_problem_size; distance++) {
        double distance_entropy_sum = 0.0;
        for (int ref_node = 0; ref_node < m_problem_size; ref_node ++) {
            distance_entropy_sum += fast_calculate_entropy(m_multiple_relation_matrix[ref_node][distance], m_sample_size);
        }
        distance_entropy_arr[distance] = distance_entropy_sum;
    }

    // calculate mean of entropy
    double mean_entropy = sum_arr(distance_entropy_arr, m_problem_size) / (m_problem_size - 1);
    // cout << "mean_entropy: " << mean_entropy << endl;
    // PrintArray(distance_entropy_arr, m_problem_size);
    // cout << endl;



    // label can used distance by boolean array
    m_can_used_distance_arr[0] = false;
    // cout << "can used distance: " << endl;
    for (int distance = 1; distance < m_problem_size; distance++) {
        if (distance_entropy_arr[distance] <= mean_entropy) {
            m_can_used_distance_arr[distance] = true;
            // cout << distance << " ";
        }
        else {
            m_can_used_distance_arr[distance] = false;
        }
    }
    // cout << endl;
 ...
 */
void CPREPRUNE::Sample(int * genes, int * templateee, int need_to_sample_gene_count)
{
    ...
    for (int kkk = already_sampled_idx_count; kkk < m_problem_size; kkk++) {
        ...
        // cout << "lowest idx = " << (lowest_entropy_edge.node_a + lowest_entropy_edge.node_a_index) % m_problem_size << endl;;



        // add new edges
        int count = 0;
        another_lowest_entropy_edge = add_edgesss(genes[sample_idx], sample_idx, edgeeeee_lst, is_sampled_node, is_sampled_idx, &count);
        // cout << "334 count: " << count << endl;;
        // cout << "lowest idx = " << (another_lowest_entropy_edge.node_a + another_lowest_entropy_edge.node_a_index) % m_problem_size << endl;;

        // cout << "edgeeeee_lst size after adding new edges: " << edgeeeee_lst.size() << endl << endl;

        if (count == 0) { continue; }
        else if (lowest_entropy_edge.entropy > another_lowest_entropy_edge.entropy) {
            lowest_entropy_edge = another_lowest_entropy_edge;
            // cout << "next sample use add edgess edgess" << endl;
        }
            ...
            // cout << "next sample use update_and_find_lowest_entropy_edge edgess" << endl;
        }
    }
...
edgeee CPREPRUNE::add_edgesss(int sampled_node_num, int sampled_node_idx, list<edgeee>& edge_lst, bool * is_sampled_node, bool * is_sampled_idx, int *count) {
    ...
    *count = 0;
    ...
    for (int i = 0; i < m_problem_size; i++) {
        if (is_sampled_idx[i] == false) {
            ...
            if (m_can_used_distance_arr[tmp_edge.distance] == false) {
                continue;
            }
    ...
    // cout << "*count :" << *count<<endl;;
    // if (count = 0) {
    //     lowest_entropy_edge.entropy = 1000;
    // }
    // cout << "lowest_entropy_edge.entropy: "<< lowest_entropy_edge.entropy << endl;
    // cout << "(add_edgess) lowest_entropy_edge.node_a: " << lowest_entropy_edge.node_a << ", lowest_entropy_edge.distance: " << lowest_entropy_edge.distance << endl;
    return lowest_entropy_edge;
}



edgeee CPREPRUNE::Build_network(int * templateee, int already_sampled_idx_count, list<edgeee>&  edge_lst, int * sampled_or_not_idx, bool * is_sampled_node) {
...
double CPREPRUNE::fast_calculate_entropy(int * arr, int summation) {
...
// update edges information and delete unsable edgesss
edgeee CPREPRUNE::update_and_find_lowest_entropy_edge(int sample_idx, int sample_node_num, list<edgeee>&  edge_lst) {
...
void CPREPRUNE::construct_probability_array(double * tmp_probability_array, bool * is_sampled_node, edgeee lowest_entropy_edge) {
