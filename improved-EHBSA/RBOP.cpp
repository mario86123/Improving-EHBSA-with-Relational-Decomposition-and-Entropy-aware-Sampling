 ...
 * Class constructor.
 */
CRBOP::CRBOP(int problem_size, int sel_size, double b_ratio, int cut_point_count)
{
 ...
 */
bool CRBOP::Learn(CPopulation * population, int size)
{   
    // population->Print();
    // cout << endl;
    ...
    cout << m_used_distance_count[1] + m_used_distance_count[m_problem_size - 1] << " ";
 ...
 */
void CRBOP::Sample(int * genes, int * templateee)
{
...
int CRBOP::Build_network(int * templateee, int already_sampled_idx_count, edgeee * edge_lst, int * sampled_or_not_idx, bool * is_sampled_node) {
...
double CRBOP::fast_calculate_entropy(double * arr, double summation) {
...
int CRBOP::update_and_find_lowest_entropy_edge(int sample_idx, int sample_node_num, int total_edge_count, edgeee * edge_lst) {
...
void CRBOP::construct_probability_array(double * tmp_probability_array, bool * is_sampled_node, int lowest_entropy_edge_num, edgeee * edge_lst) {
