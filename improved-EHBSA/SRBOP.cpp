 ...
 * Class constructor.
 */
CSRBOP::CSRBOP(int problem_size, int sel_size, double b_ratio, int cut_point_count)
{
 ...
 */
bool CSRBOP::Learn(CPopulation * population, int size)
{   
    // population->Print();
    // cout << endl;
 ...
 */
void CSRBOP::Sample(int * genes, int * templateee)
{
...
int CSRBOP::Build_network(int * templateee, int already_sampled_idx_count, edgeee * edge_lst, int * sampled_or_not_idx, bool * is_sampled_node) {
    ...
    int total_edge_count = already_sampled_idx_count * (m_problem_size - already_sampled_idx_count);
    double entropy_prob_arr[total_edge_count];
    ...
    for (int i = 0; i < already_sampled_idx_count; ++i) {                             // already sampled node
        for (int j = 0; j < (m_problem_size - already_sampled_idx_count); ++j) {      // not sampled yet node
            ...
            entropy_prob_arr[num] = 1/edge_lst[num].entropy;
    ...
    // PrintArray(entropy_prob_arr, total_edge_count);
    // cout << endl<<endl;
    lowest_entropy_edge_num = sample_from_array(entropy_prob_arr, total_edge_count);

    return lowest_entropy_edge_num;
}


double CSRBOP::fast_calculate_entropy(double * arr, double summation) {
...
int CSRBOP::update_and_find_lowest_entropy_edge(int sample_idx, int sample_node_num, int total_edge_count, edgeee * edge_lst) {

    double lowest_entropy = 100.0;
    int all_edges_lowest_entropy_num = -1;

    double entropy_prob_arr[total_edge_count];
    int edge_num_arr[total_edge_count];
    int count = 0;
    ...
    for (int edge_num = 0; edge_num < total_edge_count; edge_num++) {
        ...
        if (edge_lst[edge_num].can_be_used == true) {
            ...
            // --- update different distance edges information --- //
                ...
                // cout << edge_lst[edge_num].entropy << endl;
                
                 
                entropy_prob_arr[count] = 1/edge_lst[edge_num].entropy;
                // cout << entropy_prob_arr[count] << endl<< endl;

                edge_num_arr[count] = edge_num;
    ...
    // cout << endl<<endl;
    // PrintArray(entropy_prob_arr, count);
    // cout << endl<<endl;
    all_edges_lowest_entropy_num = sample_from_array(entropy_prob_arr, count);

    return edge_num_arr[all_edges_lowest_entropy_num];
}



void CSRBOP::construct_probability_array(double * tmp_probability_array, bool * is_sampled_node, int lowest_entropy_edge_num, edgeee * edge_lst) {
