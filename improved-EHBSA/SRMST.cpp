// Split Random MST model (split node and edge relation)
 ...
 * Class constructor.
 */
CSRMST::CSRMST(int problem_size, int sel_size, double b_ratio, int number_of_edge, string result_file_name)
{
    // cout << "e: "<< exp(1/0)<<endl;
 ...
 */
bool CSRMST::Learn(CPopulation * population, int size)
{   
    ...
    cout << "edge_node_probability_ratio: " << edge_node_probability_ratio/ m_sample_size/m_problem_size << endl;
    m_entropy_file << edge_node_probability_ratio/ m_sample_size/m_problem_size << " ";
    ...
    for (edge e: all_edge) {
        ...
        if (find_set(e.node_a, node_set) != find_set(e.node_b, node_set)) {
            ...
            if (e.distance == 1 || e.distance == m_problem_size - 1) {
    ...
    m_entropy_file << correct_edge_relation_count << " ";
...
void CSRMST::Sample(int * genes)
{
    ...
    for (int sample_count = 0; sample_count < m_problem_size - 1; sample_count++) {
        ...
        double candidate_node_entropy_list[m_problem_size];
        double candidate_edge_entropy_list[m_problem_size];

        int candidate_node_num[m_problem_size];
        int candidate_edge_num[m_problem_size];

        for (int i = 0; i < m_problem_size; i ++) {
            candidate_node_entropy_list[i] = 0;
            candidate_edge_entropy_list[i] = 0;
            candidate_node_num[i] = 0;
            candidate_edge_num[i] = 0;
        }
        ...
        for (int i = 0; i < m_problem_size; i ++) {
            if (is_sampled_node[i] == false) {
                ...
                candidate_node_entropy_list[num_of_node_relation_in_candidate_list] = pow(1 / mst_node_lst[i].entropy, 1);
                // candidate_node_entropy_list[num_of_node_relation_in_candidate_list] = exp(- mst_node_lst[i].entropy);

                // here to sum node relation entropy
                candidate_node_num[num_of_node_relation_in_candidate_list] = i;
                num_of_node_relation_in_candidate_list ++;

            }
        }
        // PrintArray(candidate_node_num, m_problem_size);
        int chosen_node_idx = sample_from_array(candidate_node_entropy_list, num_of_node_relation_in_candidate_list);
        int chosen_node_num = candidate_node_num[chosen_node_idx];
        // cout <<"chosen_node_num: "<< chosen_node_num << endl;
        // --- end of node part ---//
        ...
        for (int i = 0; i < m_max_num_of_edge; i ++) {
            ...
            if (mst_edge_lst[i].status == 1 || mst_edge_lst[i].status == 2 ) {
                ...
                candidate_edge_entropy_list[num_of_edge_relation_in_candidate_list] = pow(1 / mst_edge_lst[i].entropy, 1);
                // candidate_edge_entropy_list[num_of_edge_relation_in_candidate_list] = exp(- mst_edge_lst[i].entropy);
                ...
                candidate_edge_num[num_of_edge_relation_in_candidate_list] = i;
                num_of_edge_relation_in_candidate_list ++;
            }
        }
        // double node_vs_edge_entropy[2] = {exp(-mst_node_lst[chosen_node_num].entropy), 0};
        double node_vs_edge_entropy[2] = {1/mst_node_lst[chosen_node_num].entropy, 0};

        int chosen_edge_num;

        if (num_of_edge_relation_in_candidate_list > 0) {
            int chosen_edge_idx = sample_from_array(candidate_edge_entropy_list, num_of_edge_relation_in_candidate_list);
            // cout << "candidate_edge_num arr: ";
            // PrintArray(candidate_edge_num, num_of_edge_relation_in_candidate_list);
            // cout << "chosen_edge_idx: " << chosen_edge_idx << ", chosen_edge_num: " << candidate_edge_num[chosen_edge_idx] << endl;

            chosen_edge_num = candidate_edge_num[chosen_edge_idx];
            // node_vs_edge_entropy[1] = exp(-mst_edge_lst[chosen_edge_num].entropy);
            node_vs_edge_entropy[1] = 1/mst_edge_lst[chosen_edge_num].entropy;

            // --- end of edge part ---//

        }
        // else if (num_of_edge_relation_in_candidate_list == 0) {
        //     node_vs_edge_entropy[1] = 0;
        // }
        // double node_vs_edge_entropy[2] = {candidate_node_entropy_list[chosen_node_idx], candidate_edge_entropy_list[chosen_edge_idx]};
        edge_node_probability_ratio += (node_vs_edge_entropy[1]) / ((node_vs_edge_entropy[0]));
        // PrintArray(node_vs_edge_entropy, 2);
        // 0: node, 1: edge
        int node_or_edge = sample_from_array(node_vs_edge_entropy, 2);
        // cout << node_or_edge << endl;
        ...
        // use node arr to sample
        if (node_or_edge == 0) { // use node arr to sample

            sample_node_num = chosen_node_num;
        ...
        else { // use edge arr to sample
            int ref_node_num;

            int min_entropy_edge_num = chosen_edge_num;
            ...
            // cout << "edge, ref_node_num: " << ref_node_num << ", sample_node_num: " <<sample_node_num << ", status: " <<mst_edge_lst[min_entropy_edge_num].status<< endl;
        ...
        // PrintArray(is_sampled_node, m_problem_size);
        ...
        for (int edge_num = 0; edge_num < m_max_num_of_edge; edge_num ++) {
            ...
            if (mst_edge_lst[edge_num].node_a == sample_node_num || mst_edge_lst[edge_num].node_b == sample_node_num ) {
                // cout << "mst_edge_lst[" << edge_num << "].status: "<< mst_edge_lst[edge_num].status<< ", a: " <<mst_edge_lst[edge_num].node_a << ", b: " <<mst_edge_lst[edge_num].node_b<<  endl;
                ...
                // (status 1 -> 3) or (status 2 -> 3)  (  time complexity: O(1)  )
                    ...
                    // cout << "3) mst_edge_lst[" << edge_num << "].status: "<< mst_edge_lst[edge_num].status<< ", a: " <<mst_edge_lst[edge_num].node_a << ", b: " <<mst_edge_lst[edge_num].node_b<<  endl;
    ...
    // cout << "\ngenes[" << last_node_idx << "] = " << last_node_num << endl;
