 ...
 * Class constructor.
 */
CSoftMST::CSoftMST(int problem_size, int sel_size, double b_ratio, string result_file_name)
{
    ...
    m_how_many_trees = m_problem_size/m_problem_size;
 ...
 */
bool CSoftMST::Learn(CPopulation * population, int size)
{   
 ...
 */
void CSoftMST::Sample(int * genes)
{
    ...
    for (int sample_count = 0; sample_count < m_problem_size; sample_count++) {
        ...
        for (int i = 0; i < m_problem_size; i ++) {
            if (is_sampled_node[i] == false && is_same_category_nodes_already_sampled[i] == false) {
