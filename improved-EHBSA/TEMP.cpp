 ...
 * Class constructor.
 */
CTEMP::CTEMP(int problem_size, int sel_size, double b_ratio, int cut_point_count)
{
 ...
 */
bool CTEMP::Learn(CPopulation * population, int size)
{   
    // population->Print();
    // cout << endl;
 ...
 */
void CTEMP::Sample(int * genes, int * templateee)
{
    ...
    // cout << "all_edges_lowest_entropy_num: " << all_edges_lowest_entropy_num << endl;
    ...
    for (int kkk = 0; kkk < m_problem_size - already_sampled_idx_count; kkk++) {
        ...
        // if ((templateee[4] == 14 && templateee[5] == 11)){// || templateee[0] == 19 && templateee[1] == 10) {


        // cout << kkk <<": , lowest_entropy_edge_num: " << lowest_entropy_edge_num <<endl;
        // }
        ...
        // cout << kkk <<": , genes[" << sample_idx << "]: " << genes[sample_idx]<<endl;
        // }
...
int CTEMP::Build_network(int * templateee, int already_sampled_idx_count, edgeee * edge_lst, int * sampled_or_not_idx, bool * is_sampled_node) {
    ...
    for (int i = 0; i < already_sampled_idx_count; ++i) {                             // already sampled node
        for (int j = 0; j < (m_problem_size - already_sampled_idx_count); ++j) {      // not sampled yet node
            ...
            // cout << "m_multiple_relation_entropys[edge_lst[" << num << "].node_a][edge_lst[" << num << "].distance]: " << calculate_entropy(m_multiple_relation_matrix[edge_lst[num].node_a][edge_lst[num].distance], m_problem_size) << endl;


            // PrintArray(tmp_probability_array, m_problem_size);
            edge_lst[num].entropy = calculate_entropy_with_known_sum(tmp_probability_array, m_problem_size, edge_lst[num].element_sum);
            // if ((templateee[4] == 14 && templateee[5] == 11)){// || templateee[0] == 19 && templateee[1] == 10) {


            // cout << "edge_lst[" << num << "].entropy: " << edge_lst[num].entropy << endl << endl;
            //             if(num == 26) PrintArray(tmp_probability_array, m_problem_size);
    ...
    // cout << "edge_lst[lowest_entropy_edge_num].node_a]: " << edge_lst[lowest_entropy_edge_num].node_a;
    // cout << ", edge_lst[lowest_entropy_edge_num].distance]: " << edge_lst[lowest_entropy_edge_num].distance << endl;

    // for (int k = 0; k < m_problem_size; ++k) {
                
    //     // if that node num is not sampled yet
    //     if (is_sampled_node[k] == false) {
    //         cout << m_multiple_relation_matrix[edge_lst[lowest_entropy_edge_num].node_a][edge_lst[lowest_entropy_edge_num].distance][k] << " ";
    //         // edge_lst[num].element_sum += m_multiple_relation_matrix[edge_lst[num].node_a][edge_lst[num].distance][k];
    //     }
    //     // else if that node num is already sampled
    //     else {
...
int CTEMP::update_and_find_lowest_entropy_edge(int sample_idx, int sample_node_num, int total_edge_count, edgeee * edge_lst) {
    ...
    for (int edge_num = 0; edge_num < total_edge_count; edge_num++) {
        ...
        if (edge_lst[edge_num].can_be_used == true) {
            ...
            // --- update different distance edges information --- //
                ...
                // if ((edge_num ==7 || edge_num == 48) && sample_idx == 14) {

                // cout << "edge_lst[" << edge_num << "].entropy: " << edge_lst[edge_num].entropy << ", edge_lst[edge_num].element_sum: " << edge_lst[edge_num].element_sum<<  "edge_lst[edge_num].entropy: " << edge_lst[edge_num].entropy<< endl;
                // cout << "diff: " << edge_lst[edge_num].entropy - lowest_entropy << endl;
                ...
                // cout << "edge_lst[" << edge_num << "].entropy: " << edge_lst[edge_num].entropy << endl;
                // cout << "diff: " << edge_lst[edge_num].entropy - lowest_entropy << endl;
                // }
...
void CTEMP::construct_probability_array(double * tmp_probability_array, bool * is_sampled_node, int lowest_entropy_edge_num, edgeee * edge_lst) {
