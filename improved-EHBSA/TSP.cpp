...
double TSP::CalculateGEODistance(double latitudeX, double latitudeY, double longitudeX, double longitudeY)
{
	...
	else
	{
		...
		do
		{
			...
			if (sub!=""){
				...
				// printf("%d ", m_distance_matrix[j][i]);
				// cout << sub << endl;
		    	if (sub=="0")
		    	{
		    		i++;
...
double TSP::Evaluate(int * genes)
{
	...
	for (int i=0;i<m_size;i++)
	{
		...
		// cout << IDCityA<< " " << IDCityB <<" distAB: " << distAB << endl;
